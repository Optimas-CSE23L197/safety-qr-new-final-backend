...
export const authenticate = asyncHandler(async (req, _res, next) => {
  ...
  else if (req.headers.authorization && BEARER_REGEX.test(req.headers.authorization)) {
  ...
  if (cachedStatus === null) {
    ...
    if (dbBlacklist && new Date(dbBlacklist.expires_at) > new Date()) {
      ...
      const ttlSecs = Math.ceil((new Date(dbBlacklist.expires_at) - Date.now()) / 1000);
      if (ttlSecs > 0) {
  ...
  if (cachedSession) {
    ...
    if (session) {
      redis.setex(sessionKey, SESSION_TTL, JSON.stringify(session)).catch(() => {});
    }
  }

  if (!session) throw ApiError.unauthorized('Session not found');
  if (!session.is_active)
    throw ApiError.unauthorized(`Session ended: ${session.revoke_reason ?? 'unknown'}`);
  if (new Date(session.expires_at) < new Date()) throw ApiError.unauthorized('Session expired');
  ...
  throttledLastActive(payload.sessionId).catch(e =>
    logger.warn({ sessionId: payload.sessionId, err: e.message }, 'Failed to update last_active_at')
  );
...
function isUserActive(user, role) {
  if (role === 'PARENT_USER') return user.status === 'ACTIVE' && !user.deleted_at;
