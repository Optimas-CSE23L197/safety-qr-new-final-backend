...
async function calculateBehavioralScore(ip, requestData) {
  ...
  if (requestData.userAgent) {
    ...
    const isNormalMobile = NORMAL_PATTERNS.MOBILE_UA.some((p) =>
      ua.includes(p.toLowerCase()),
    );
  ...
  if (requestData.role && NORMAL_PATTERNS.NORMAL_ENDPOINTS[requestData.role]) {
    ...
    const isNormalEndpoint = normalEndpoints.some((e) =>
      requestData.path.startsWith(e),
    );
    if (!isNormalEndpoint && !requestData.path.startsWith("/api/auth")) {
      score += 10;
      reasons.push(
        `Unusual endpoint for role ${requestData.role}: ${requestData.path}`,
      );
    }
  }
...
export async function recordSuccessfulAuth(ip, userId, role) {
  ...
  if (score > 0) {
    ...
    if (isBlocked && newScore < BEHAVIOR_THRESHOLDS.SUSPICIOUS) {
      ...
      logger.info(
        { ip, userId, role },
        `IP ${ip} unblocked after successful auth`,
      );
    }
  }
}
...
export const behavioralSecurity = asyncHandler(async (req, res, next) => {
  ...
  if (isBlocked) {
    ...
    logger.warn(
      { ip, score: blockData.score },
      "Behaviorally blocked IP attempted access",
    );
  ...
  if (score >= BEHAVIOR_THRESHOLDS.SUSPICIOUS) {
    logger.warn(
      ...
      `⚠️ Suspicious activity (score: ${score})`,
    );

    if (
      score >= BEHAVIOR_THRESHOLDS.SUSPICIOUS &&
      score < BEHAVIOR_THRESHOLDS.BLOCK
    ) {
...
export async function getBehavioralReport() {
  ...
  return {
    totalSuspicious: reports.filter(
      (r) => r.score >= BEHAVIOR_THRESHOLDS.SUSPICIOUS,
    ).length,
    totalBlocked: reports.filter((r) => r.score >= BEHAVIOR_THRESHOLDS.BLOCK)
      .length,
