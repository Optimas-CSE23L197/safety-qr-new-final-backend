...
export const tokenGenerationWorker = new Worker(
  ...
  async (job) => {
    ...
    logger.info(
      `[worker:token] START job=${job.id} order=${orderId} count=${cardCount}`,
    );
    ...
    if (alreadyGenerated >= cardCount) {
      logger.info(
        `[worker:token] Already complete: order=${orderId} existing=${alreadyGenerated}`,
      );
    ...
    const itemsToProcess = isPreDetails
      ? (order.items ?? []).slice(startIndex)
      : [];
    ...
    const cardNumberCandidates = Array.from(
      { length: createdTokens.length + 20 },
    ...
    for (let i = 0; i < createdTokens.length; i += QR_CONCURRENCY) {
      ...
      const results = await Promise.all(
        batch.map(async (token, j) => {
          ...
          try {
            ...
            logger.error(
              `[worker:token] QR failed token=${token.id}: ${err.message}`,
            );
      ...
      // Progress reporting
      if (
        processed % PROGRESS_PUBLISH_EVERY === 0 ||
        ...
        await Promise.all([
          ...
          pipelineRepo.updateStepProgress(
            jobExec.step_execution_id,
            pct,
            detail,
          ),
    ...
    await prisma.tokenBatch.update({
      ...
      data: {
        ...
        error_log:
          failedTokenIds.length > 0 ? { failed: failedTokenIds } : null,
      },
    });
