...
bullRedisConnection.on('close', () => {
  logger.warn({ type: 'bullmq_redis_close' }, 'BullMQ Redis: connection closed');
});
...
bullRedisConnection.on('end', () => {
  logger.warn({ type: 'bullmq_redis_end' }, 'BullMQ Redis: connection permanently closed');
});
...
export const tokenGenerationEvents = new QueueEvents(QUEUE_NAMES.TOKEN_GENERATION, {
  connection: bullRedisConnection,
});
...
export const enqueueTokenGeneration = async ({
  ...
  logger.info(`[queue] Enqueued token generation: order=${orderId} jobExec=${jobExecution.id}`);
 ...
 */
export const enqueueCardDesign = async ({ stepExecutionId, orderId, schoolId, adminId, ip }) => {
  const jobExecution = await pipelineRepo.createJobExecution({
    stepExecutionId,
    orderId,
  ...
  logger.info(`[queue] Enqueued card design: order=${orderId} jobExec=${jobExecution.id}`);
...
export const getQueueHealth = async () => {
  const [tokenStats, designStats] = await Promise.all([
    tokenGenerationQueue.getJobCounts('active', 'waiting', 'failed', 'completed', 'delayed'),
    cardDesignQueue.getJobCounts('active', 'waiting', 'failed', 'completed', 'delayed'),
  ]);
...
export const drainDeadJob = async (jobExecutionId, bullJobId) => {
  const job =
    (await tokenGenerationQueue.getJob(bullJobId)) ?? (await cardDesignQueue.getJob(bullJobId));
