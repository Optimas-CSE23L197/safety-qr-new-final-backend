...
import { publishNotification, publishFailure } from './events/event.publisher.js';
...
export function createCancelWorker() {
    ...
    async job => {
      ...
      try {
        if (!stepExecutionId) {
          ...
          stepExecution = await beginStepExecution(pipeline.id, orderId, 'CANCEL', 'system');
        } else {
  ...
  worker.on('completed', job => logger.info({ msg: 'Cancel worker job completed', jobId: job.id }));
