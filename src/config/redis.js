...
function createRedisClient(name, options) {
  ...
  if (ENV.REDIS_SENTINEL === true && ENV.REDIS_SENTINEL_NODES) {
    ...
    logger.info(
      { type: "redis_sentinel", client: name },
    ...
    logger.info(
      { type: "redis_cluster", client: name },
      `Redis [${name}]: using Cluster mode`,
    );
  } else {
    client = new Redis(ENV.REDIS_URL, options);
    logger.info(
      { type: "redis_single", client: name },
      `Redis [${name}]: using single node mode`,
    );
  }
 ...
 */
export const redis =
  g.__redis ?? (g.__redis = createRedisClient("main", buildHttpOptions()));
...
export const middlewareRedis =
  g.__middlewareRedis ??
  (g.__middlewareRedis = createRedisClient(
    "middleware",
...
export const workerRedis =
  g.__workerRedis ??
  (g.__workerRedis = createRedisClient("worker", buildWorkerOptions()));
