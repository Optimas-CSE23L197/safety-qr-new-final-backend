...
export async function getVendorMetrics(vendorId, fromDate, toDate) {
  ...
  const completedCards = completedOrders.reduce(
    (sum, o) => sum + o.card_count,
    0,
  );
  ...
  for (const order of completedOrders) {
    if (order.print_complete_at) {
      const turnaround =
        order.print_complete_at.getTime() - order.created_at.getTime();
  ...
  const avgTurnaroundDays =
    completedOrders.length > 0
      ? Math.round(
          totalTurnaround / completedOrders.length / (1000 * 60 * 60 * 24),
        )
      : 0;
