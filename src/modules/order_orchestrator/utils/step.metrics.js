...
export async function getStepMetrics(step, period = "hour") {
  ...
  if (period === "minute") {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push(
        `${METRICS_PREFIX}:${step}:daily:${date.toISOString().slice(0, 10)}`,
      );
    }
    keys.metrics = days;
    ...
    const [total, success, failure, avgDuration, p50, p95, p99] =
      await Promise.all([
    ...
    return {
      ...
      successRate: total
        ? ((parseInt(success || "0", 10) / parseInt(total, 10)) * 100).toFixed(
            2,
          )
        : 0,
