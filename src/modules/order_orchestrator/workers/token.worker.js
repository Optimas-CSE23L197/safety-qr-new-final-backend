...
async function processTokenGeneration(orderId, stepExecutionId, jobId) {
  ...
  console.log(
    `Existing tokens: ${existingTokens}, Expected: ${order.card_count}`,
  );
  ...
  const cardNumbers = batchGenerateCardNumbers(
    order.school.serial_number,
    total,
  );
  ...
  for (let batchStart = 0; batchStart < total; batchStart += BATCH_SIZE) {
    ...
    for (let j = 0; j < tokens.length; j += PARALLEL_QR) {
      ...
      const chunkResults = await Promise.allSettled(
        chunk.map(async (token, idx) => {
          ...
          console.log(
            `      📷 Token ${token.id}: QR buffer size=${qrBuffer.length}`,
          );
      ...
      for (const settled of chunkResults) {
        if (settled.status === "fulfilled") {
          ...
          console.log(
            `      ✅ QR generated for token ${settled.value.token.id}`,
          );
        } else {
          console.error(
            `      ❌ QR generation failed: ${settled.reason?.message}`,
          );
          logger.error({
    ...
    console.log(
      `📊 Batch progress: generated=${generatedCount}/${total}, failed=${failedCount}`,
    );
  }
  ...
  const batchStatus =
    failedCount === 0 ? "COMPLETE" : generatedCount > 0 ? "PARTIAL" : "FAILED";
  ...
  console.log(
    `Summary: ${generatedCount} tokens generated, ${failedCount} failed`,
  );
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      if (event !== "ADVANCE_PAYMENT_RECEIVED" && event !== "ORDER_APPROVED") {
        console.log(
          `⏭️ Skipping event: ${event} - not a token generation trigger`,
        );
        return {
          skipped: true,
