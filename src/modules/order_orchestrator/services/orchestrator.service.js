...
async function safePublishEvent(event, orderId, payload) {
  try {
    ...
    try {
      await markStalled(
        orderId,
        `Event publish failed: ${event} — ${err.message}`,
      );
    } catch (stallErr) {
      logger.error({
 ...
 * @returns {Promise<object>}
 */
export const approveOrderOrchestration = async (
  orderId,
  actor,
  options = {},
) => {
 ...
 * @returns {Promise<object>}
 */
export const processPaymentOrchestration = async (
  orderId,
  paymentData,
  actor,
) => {
  ...
  if (!lockAcquired) {
    throw new Error(
      "Another process is already processing payment for this order",
    );
  }
 ...
 * @returns {Promise<object>}
 */
export const cancelOrderOrchestration = async (
  orderId,
  actor,
  options = {},
) => {
 ...
 * @returns {Promise<object>}
 */
export const retryFailedStepOrchestration = async (
  orderId,
  step,
  actor,
  options = {},
