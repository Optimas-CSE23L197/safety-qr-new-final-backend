...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(
      new ApiResponse(200, result, result.success ? 'Pipeline resumed' : 'Pipeline not stalled')
    );
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const idempotencyKey = req.headers['x-razorpay-signature'] || `${event}:${Date.now()}`;
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const idempotencyKey = req.headers['x-shiprocket-signature'] || `${event}:${Date.now()}`;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    res.json(
      new ApiResponse(200, { status: 'healthy', timestamp: new Date().toISOString() }, 'OK')
    );
  })
);
