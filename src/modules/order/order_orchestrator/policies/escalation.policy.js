...
export const shouldEscalateStepFailure = (step, retryCount, error) => {
  ...
  const isCriticalError = criticalErrorMessages.some(msg => errorMsg.includes(msg));
...
export const shouldEscalateStalledPipeline = (step, stalledAt) => {
  const severity = STEP_SEVERITY[step] || 'MEDIUM';
  const thresholdMinutes = ESCALATION_CONFIG.STALLED_THRESHOLD_MINUTES[severity];
 ...
 * @param {string} params.reason
 */
export const escalateStalledPipeline = async ({ orderId, step, stalledAt, severity, reason }) => {
  logger.warn({
  ...
  const stalledMinutes = Math.floor((Date.now() - new Date(stalledAt).getTime()) / 60000);
