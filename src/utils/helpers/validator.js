...
export function isValidOtp(otp, length = 6) {
  ...
  return valid
    ? { valid: true }
    : { valid: false, reason: `OTP must be exactly ${length} digits` };
}
...
export function isValidCardNumber(cardNumber) {
  ...
  const valid = /^[A-Z0-9]{6,20}$/.test(
    String(cardNumber).toUpperCase().replace(/\s/g, ""),
  );
  return valid
    ? { valid: true }
    : { valid: false, reason: "Invalid card number format" };
}
...
export function isValidGstin(gstin) {
  ...
  return valid
    ? { valid: true }
    : { valid: false, reason: "Invalid GSTIN format" };
}
...
export function isValidRazorpayOrderId(id) {
  ...
  return valid
    ? { valid: true }
    : { valid: false, reason: "Invalid Razorpay order ID" };
}
...
export function isValidRazorpayPaymentId(id) {
  ...
  return valid
    ? { valid: true }
    : { valid: false, reason: "Invalid Razorpay payment ID" };
}
...
export function isValidUuid(id) {
  ...
  return isUuid || isCuid2
    ? { valid: true }
    : { valid: false, reason: "Invalid ID format" };
}
...
export function isNonEmptyString(value, fieldName = "Field") {
  ...
  return valid
    ? { valid: true }
    : { valid: false, reason: `${fieldName} cannot be empty` };
}
