...
// Used for: Security alert when a new device logs in
...
const s = {
    ...
    headerIcon: { fontSize: '36px', marginBottom: '8px', display: 'block' },
    ...
    detailBox: { backgroundColor: c.redLight, border: `1px solid #f5d0d0`, borderRadius: '10px', padding: '16px 20px', marginBottom: '24px' },
    ...
    warningText: { fontSize: '13px', color: c.red, fontWeight: '600', marginBottom: '24px' },
...
export default function DeviceLoginEmail({ name = 'User', device = 'Unknown device', location = 'Unknown location', time = 'Just now' }) {
    return (
        <Html lang="en"><Head />
            <Preview>New login detected on your RESQID account</Preview>
            <Body style={s.body}>
                <Section style={s.wrapper}>
                    <Container style={s.card}>
                        <Section style={s.header}>
                            <span style={s.headerIcon}>🔐</span>
                            <Heading style={s.headerTitle}>New Login Detected</Heading>
                        </Section>
                        <Section style={s.bodySection}>
                            <Text style={s.greeting}>Hi {name},</Text>
                            <Text style={s.text}>A new device just logged into your RESQID account. If this was you, no action is needed.</Text>
                            <Section style={s.detailBox}>
                                <Section style={s.detailRow}>
                                    <span style={s.detailLabel}>Device</span>
                                    <Text style={s.detailValue}>{device}</Text>
                                </Section>
                                <Section style={s.detailRow}>
                            ...
                            <Text style={s.warningText}>If this wasn't you, secure your account immediately.</Text>
                            <Text style={s.text}>Need help? Contact <Link href="mailto:support@getresqid.in" style={s.link}>support@getresqid.in</Link></Text>
                        </Section>
